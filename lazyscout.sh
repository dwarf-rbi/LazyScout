...
banner() {
    ...
    echo ""
    echo -e "\e[35m             - Automated DyslexSec - \e[0m"
}
...
usage() {
    echo "Usage: $0 [-useamass] <target domain | file with multiple wildcard domains>"
    echo "Options:"
    echo "  -h         Show this help message"
    echo "  -useamass  Enable Amass for subdomain discovery (disabled by default)"
    echo "Examples:"
    echo "  $0 example.com"
    echo "  $0 -useamass domains.txt"
    exit 0
}
...
if $USE_AMASS; then
    ...
    echo "[*] Skipping Amass (use -useamass to enable it)..."
...
httpx -silent -status-code -l $output_dir/all_subdomains.txt -o $output_dir/httpx_all.txt
...
if [[ -s $output_dir/httpx_200.txt ]]; then
    ...
    katana -list $output_dir/httpx_200.txt -silent -o $output_dir/katana_urls.txt
    gospider -S $output_dir/httpx_200.txt -o $output_dir/gospider_output -t 10 -c 10 --quiet &> /dev/null
else
...
find $output_dir/gospider_output -name "*.txt" -exec cat {} \; | grep -oP '(http|https)://\S+' | sort -u > $output_dir/gospider_urls.txt
...
grep -Ei "admin|login|register|forgotpassword" $output_dir/all_urls.txt > $output_dir/admin_urls.txt
...
echo -e "\e[32m[*] Total execution time: $TOTAL_TIME seconds\e[0m"
echo -e "\e[32m===================================================\e[0m"